...
if [ -n "$(pidof Zinnid)" ] || [ -e "Zinnid" ] ; then
  echo -e "${RED}Old Zinnid is already installed - removing previous installation.${NC}"
  systemctl stop Zinni.service
  systemctl disable Zinni.service
  sleep 3
  rm /etc/systemd/system/Zinni.service
  systemctl daemon-reload
fi
}
